<template>
  <div>
    <el-form inline :model="listQuery" ref="queryForm" label-width="100px">
      ...
      <el-form-item label="指令类型" prop="type">
        <el-select v-model="listQuery.type"
                   placeholder="请选择指令类型"
                   :loading="commandTypeLoading"
                   @focus="fetchCommandTypes">
          <el-option
            v-for="item in commandTypes"
            :key="item.id"
            :label="item.label"
            :value="item.id"
          >
      ...
      <el-form-item>
        ...
        <el-button type="primary" @click="fetchCommands">查询</el-button>
      </el-form-item>
    ...
    <el-table :data="list" style="width: 100%;padding-top: 15px;" v-loading="listLoading">
      ...
      <el-table-column label="指令类型" min-width="100">
        <template slot-scope="scope">
          {{ scope.row.type.label }}
        </template>
      </el-table-column>
      <el-table-column label="指令内容" min-width="100" show-overflow-tooltip=“true”>
        <template slot-scope="scope">
          {{ scope.row.body }}
        </template>
      </el-table-column>
...
<script>
  ...
  import {getCommands, getCommandTypes} from "@/api/commandmgr";
  ...
  export default {
    ...
    methods: {
      ...
      fetchCommands() {
        ...
        getCommands(this.listQuery.name,
          this.listQuery.type,
          startTime,
          endTime,
          this.listQuery.page,
       ...
       */
      fetchCommandTypes() {
        if (this.commandTypes.length > 0) return
        this.commandTypeLoading = true
        getCommandTypes().then(res => {
          this.commandTypes = res['data']['list']
